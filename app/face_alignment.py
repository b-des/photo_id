...
def rotation_detection_dlib(img, mode, show=False):
    ...
    predictor = dlib.shape_predictor(ROOT_DIR + "/data/shape_predictor_5_face_landmarks.dat")
    ...
    if len(rects) > 0:
        for rect in rects:
            ...
            if mode:
                ...
                img = np.array(img.rotate(angle, fillcolor='white'))
                #img = img.rotate(2)
            #show_img(img)
...
def rotation_detection_opencv(img, mode, show=False):
    ...
    faceCascade = cv2.CascadeClassifier(ROOT_DIR + "/data/haarcascade_mcs_nose.xml")
    ...
    eyes_cascade = cv2.CascadeClassifier(ROOT_DIR + "/data/haarcascade_eye.xml")
    fase_cascade = cv2.CascadeClassifier(ROOT_DIR + "/data/haarcascade_frontalface_default.xml")
    nose_cascade = cv2.CascadeClassifier(ROOT_DIR + "/data/haarcascade_frontalface_default.xml")
...
def face_alignment(img):
    return rotation_detection_dlib(img, 0, False)
